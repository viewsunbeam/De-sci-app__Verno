...
async function main() {
  ...
  try {
    ...
    const ConstraintManager = await ethers.getContractFactory(
      "ConstraintManager"
    );
    ...
    console.log(
      "✅ ConstraintManager部署完成:",
      deployedContracts.constraintManager
    );

    // 2.3 部署 DataFeatureExtractor 数据特征提取合约
    console.log("\n📊 部署DataFeatureExtractor合约...");
    const DataFeatureExtractor = await ethers.getContractFactory(
    ...
    deployedContracts.dataFeatureExtractor =
      await dataFeatureExtractor.getAddress();
    console.log(
      "✅ DataFeatureExtractor部署完成:",
      deployedContracts.dataFeatureExtractor
    );

    // 2.4 部署 ResearchDataVerifier 科研数据验证合约
    console.log("\n🔬 部署ResearchDataVerifier合约...");
    const ResearchDataVerifier = await ethers.getContractFactory(
    ...
    deployedContracts.researchDataVerifier =
      await researchDataVerifier.getAddress();
    console.log(
      "✅ ResearchDataVerifier部署完成:",
      deployedContracts.researchDataVerifier
    );
    ...
    const InfluenceRanking = await ethers.getContractFactory(
      "InfluenceRanking"
    );
    ...
    console.log(
      "5.  DataFeatureExtractor:",
      deployedContracts.dataFeatureExtractor
    );
    console.log(
      "6.  ResearchDataVerifier:",
      deployedContracts.researchDataVerifier
    );
