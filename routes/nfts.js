...
router.post('/mint', upload.single('coverImage'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      // Use the actual uploaded file path for local development
    ...
    const metadata = {
      ...
      image: coverImage || previewImageCID || generateDefaultImage(title),
      assetType,
    ...
    // Handle project_id for different asset types to satisfy NOT NULL constraint
    let projectId;
    if (assetType === 'Project') {
      projectId = selectedAsset;
    } else {
      // For Dataset and Publication NFTs, use a default project ID or create one
      // Use project ID 4 (existing project) as default for non-project NFTs
    ...
    // Log the NFT mint activity
    await ActivityLogger.logNFTMinted(
      user.id,
      newNFT.owner_username,
      newNFT.id,
      title,
      req
    );
