            ...
            <n-card title="Proof Information" class="info-card">
              <div v-if="proofInfo.proof_id" class="proof-info">
                ...
                <div class="proof-details-grid">
                  ...
                  <div class="detail-item" v-if="proofInfo.verification_count !== undefined">
                    <span class="detail-label">Verification Count</span>
                    <span class="detail-value">{{ proofInfo.verification_count }} times</span>
                  </div>
...
const verifyProof = async () => {
  if (!proofInfo.value.proof_id) {
    message.warning('No ZK proof available to verify')
    return
  }

  isVerifying.value = true
  try {
    // Verify the proof using the public inputs from the proof info
    const response = await axios.post(
      `http://localhost:3000/api/datasets/zk-proof/${proofInfo.value.proof_id}/verify`,
      { public_inputs: proofInfo.value.public_inputs || [] }
    )

    const result = response.data
    if (result.valid) {
      message.success(`🎉 ZK Proof验证成功！\n验证次数: ${result.verification_count}`)
      // Update the verification count in the UI
      proofInfo.value.verification_count = result.verification_count
    } else {
      message.error('❌ ZK Proof验证失败')
    }

  } catch (error) {
    console.error('Proof verification failed:', error)
    message.error('验证过程出错: ' + (error.response?.data?.error || error.message))
  } finally {
