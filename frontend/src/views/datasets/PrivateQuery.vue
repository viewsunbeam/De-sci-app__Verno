  ...
  try {
    ...
    if (!currentUser || !currentUser.account) {
      ...
      return
         ...
         // Find ZK proof for current dataset
     const datasetId = route.query.dataset_id
     if (!datasetId) {
       message.error('No dataset ID found')
       return
     }

    // Get ZK proof details
    const response = await axios.get(
      `http://localhost:3000/api/datasets/${datasetId}/zk-proof?wallet_address=${currentUser.account}`
    )
    
    const proofInfo = response.data
    if (!proofInfo.proof_id) {
      message.warning('No ZK proof found for this dataset')
      return
    }

    // Verify the proof using the correct public inputs
    const verifyResponse = await axios.post(
      `http://localhost:3000/api/datasets/zk-proof/${proofInfo.proof_id}/verify`,
      { public_inputs: proofInfo.public_inputs }
    )

    const verifyResult = verifyResponse.data
    if (verifyResult.valid) {
      message.success(`ZK Proof验证成功! (验证次数: ${verifyResult.verification_count})`)
    } else {
