...
import { HomeOutline, PersonOutline, FolderOpenOutline, DocumentTextOutline, LogOutOutline, CompassOutline, ServerOutline, ShieldCheckmarkOutline, DiamondOutline, CheckmarkDoneOutline, TerminalOutline } from '@vicons/ionicons5';
...
const fetchUserRole = async (walletAddress) => {
  ...
  try {
    const response = await axios.get(`http://localhost:3000/api/users/wallet/${walletAddress}`);
    if (response.data) {
      const roleMap = {
        'researcher': '研究员',
        'reviewer': '评审员',
      ...
      userRole.value = roleMap[response.data.user_role] || '研究员';
    }
  } catch (error) {
...
onMounted(() => {
  ...
  if (storedUser) {
    ...
    if (user.value?.wallet_address) {
      fetchUserRole(user.value.wallet_address);
    }
  }
});
